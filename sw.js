 ...
 */
const VERSIONE = 'scudo-campo-v149';
...
const RISORSE = [
  ...
  './js/calcoli.js',
  './js/luoghi.js',
  './js/conferma_pacchetto.js',
  './js/giro_presidi.js',
  './js/nome_presidio.js',
  './js/mappa.js',
  './js/riepilogo_home.js',
  './js/admin.js',
  './js/ubicazione.js',
  './js/sposta.js',
  './js/posizioni_proposte.js',
  './js/senza_posizione.js',
  './js/anomalia_testo.js',
  './js/pdf.js',
  './js/verbale.js',
  './js/spazio.js',
  './js/firmatari.js',
...
self.addEventListener('message', (ev) => {
  ...
  // Chi sta eseguendo, detto da chi esegue.
  //
  // ⛔ È l'unica risposta che non può sbagliare, e serviva: la pagina lo leggeva
  // da `caches.keys()`, prendendo la PRIMA chiave `scudo-campo-v*`. Durante un
  // aggiornamento le cache sono DUE — quella attiva e quella appena installata
  // in attesa — e l'ordine di `caches.keys()` non è garantito. La diagnostica
  // poteva quindi annunciare la revisione NUOVA mentre la pagina eseguiva la
  // vecchia, cioè dire «sei aggiornato» proprio a chi non lo era. Un worker,
  // invece, sa il proprio nome.
  if (ev.data && ev.data.tipo === 'VERSIONE' && ev.ports && ev.ports[0]) {
    ev.ports[0].postMessage(VERSIONE);
  }
});
