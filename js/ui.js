...
export function toast(messaggio, variante = '', durataMs = 3400, { onAnnulla } = {}) {
  ...
  let usato = false;
  const n = el('button', {
    class: `toast toast-annullabile ${variante}`.trim(), type: 'button', dataset: { id },
    'aria-label': `${messaggio}. Tocca per annullare`,
    onclick: () => {
      if (usato) return;
      usato = true;
      n.remove();
  ...
  wrap.append(n);
  setTimeout(() => { n.remove(); }, Math.max(durataMs, 7000));
  return n;
}
...
export function sceltaCercabile({ voci, valore, placeholder = 'Cerca…', onCambia }) {
  ...
  disegna();
  return { nodo, valore: () => scelto, apri: () => { aperto = true; aggiorna(); } };
}
