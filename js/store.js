...
// L'errore della sola COPIA DI EMERGENZA quando il rilievo sta nel database
// (25/09/2026): è un'altra cosa dal salvataggio che non riesce, e confonderli
// diceva «esporta subito» a chi aveva il lavoro al sicuro.
let erroreEmergenza = null;
...
/**
 * Quanto spazio occupa Scudo Campo su questo dispositivo (25/09/2026, richiesta
 * dell'operatore: «nella pagina dati dovremmo mostrare la dimensione
 * dell'IndexedDB, così che se è troppo grande lo capiamo»).
 *
 * Due misure diverse, e servono entrambe:
 *  * il CONTENUTO: rilievo e giornale, misurati in byte del loro JSON. Non è
 *    l'ingombro su disco — ogni browser lo scrive a modo suo — ma è la grandezza
 *    di quello che l'app riscrive a ogni salvataggio, ed è confrontabile fra un
 *    telefono e l'altro;
 *  * quello che il BROWSER dice (`navigator.storage.estimate`): quanto usa il
 *    sito — dati e app in cache — e quanto spazio gli concede. È il limite vero.
 *    Può non esserci (Safari prima di iOS 17): allora si dice che il browser tace.
 * `localStorage` in caratteri: il suo limite si conta così.
 */
export async function misuraSpazio() {
  const byte = (v) => {
    try { return new TextEncoder().encode(JSON.stringify(v ?? null)).length; } catch { return null; }
  };
  const m = {
    modalita, errore: ultimoErrore ? String(ultimoErrore.message || ultimoErrore) : null,
    erroreEmergenza: erroreEmergenza ? String(erroreEmergenza.message || erroreEmergenza) : null,
    rilievo: null, rilievoCaratteri: null, emergenzaCaratteri: 0, giornale: null, eventi: null, localStorage: 0,
    uso: null, quota: null, perIndexedDB: null, persistente: null,
    // App installata sulla schermata Home o pagina del browser: su iPhone
    // hanno dati SEPARATI, e l'operatore deve sapere in quale dei due è.
    installata: null,
  };
  try {
    m.installata = Boolean((globalThis.matchMedia && globalThis.matchMedia('(display-mode: standalone)').matches)
      || (typeof navigator !== 'undefined' && navigator.standalone === true));
  } catch { /* non si sa */ }
  if (modalita === 'idb' && db) {
    try {
      const [dataset, giornale] = await Promise.all([kvGet('dataset'), attendi(tx(STORE_JOURNAL, 'readonly').getAll())]);
      m.rilievo = byte(dataset);
      try { m.rilievoCaratteri = JSON.stringify(dataset ?? null).length; } catch { /* sotto */ }
      m.giornale = byte(giornale);
      m.eventi = (giornale || []).length;
    } catch { /* si mostrano le altre misure */ }
  } else {
    const d = leggiLocalStorage();
    m.rilievo = byte(d);
    try { m.rilievoCaratteri = JSON.stringify(d ?? null).length; } catch { /* sotto */ }
  }
  // La copia di emergenza si misura in CARATTERI: è così che localStorage conta.
  try { m.emergenzaCaratteri = (localStorage.getItem(LS_DATASET) || '').length; } catch { /* niente storage */ }
  try {
    for (let i = 0; i < localStorage.length; i += 1) {
      const k = localStorage.key(i);
      if (k && k.startsWith(LS_PREFIX)) m.localStorage += k.length + (localStorage.getItem(k) || '').length;
    }
  } catch { /* niente storage */ }
  try {
    if (navigator.storage && navigator.storage.estimate) {
      const e = await navigator.storage.estimate();
      m.uso = Number.isFinite(e.usage) ? e.usage : null;
      m.quota = Number.isFinite(e.quota) ? e.quota : null;
      m.perIndexedDB = e.usageDetails && Number.isFinite(e.usageDetails.indexedDB) ? e.usageDetails.indexedDB : null;
    }
    if (navigator.storage && navigator.storage.persisted) m.persistente = await navigator.storage.persisted();
  } catch { /* il browser non lo dice */ }
  return m;
}
...
export async function salvaConEvento(dataset, eventi) {
  ...
  if (modalita !== 'idb' || !db) {
    // ⛔ Il risultato DICE se è riuscito (25/09/2026). Prima si rispondeva
    // `ok: true` anche con localStorage pieno: misurato su Chrome, ci stanno
    // circa 5,24 milioni di caratteri e il rilievo appena caricato ne occupa
    // 2,7 — un giro intero (~8) non ci sta, e l'app diceva «salvato» mentre
    // il lavoro restava solo in memoria.
    if (!salvaLocalStorage(dataset)) {
      return { ok: false, modalita, errore: 'spazio della copia di emergenza esaurito' };
    }
  ...
  try {
    ...
    ultimoErrore = e;
    modalita = 'localstorage';
    const riuscito = salvaLocalStorage(dataset);
    return { ok: riuscito, modalita, degradato: true, errore: String(e.message || e) };
  }
}
...
// --------------------------------------------------------------------------- //
/**
 * Il rilievo in localStorage. `true` se è stato scritto, `false` se no.
 *
 * Due ruoli, e il fallimento vuol dire cose diverse:
 *  * PRIMARIA (il database del browser non c'è): è l'unica copia. Se non ci sta
 *    si tiene quella di prima — meglio vecchia che niente — e si dice
 *    all'operatore che non è salvato;
 *  * SECONDARIA (`secondaria: true`, il rilievo sta nel database): è una copia di
 *    emergenza scritta all'uscita, per il giorno in cui il database non
 *    rispondesse all'avvio. ⛔ Se non ci sta si TOGLIE (25/09/2026): restava
 *    quella dell'ultima volta che ci stava — giorni prima, a metà giro — e un
 *    avvio con il database che non risponde l'avrebbe caricata come se fosse il
 *    rilievo di oggi, senza dire niente. Misurato sull'iPhone di prova con il
 *    pacchetto dell'operatore: 5,1 MB di rilievo, «The quota has been exceeded».
 *    Il lavoro resta nel database e nella scatola nera (gli ultimi eventi), che
 *    con la copia vecchia tolta ha di nuovo spazio.
 */
export function salvaCopia(dataset, { secondaria = false } = {}) {
  try {
    localStorage.setItem(LS_DATASET, JSON.stringify(dataset));
    if (secondaria) erroreEmergenza = null;
    return true;
  } catch (e) {
    if (secondaria) {
      erroreEmergenza = e;
      try { localStorage.removeItem(LS_DATASET); } catch { /* niente storage */ }
    } else {
      // Primaria: si conserva almeno la scatola nera, e si DICE — chi chiama
      // deve saperlo, o l'app mostra «salvato» su un lavoro perso.
      ultimoErrore = e;
    }
    return false;
  }
}
const salvaLocalStorage = (dataset) => salvaCopia(dataset);
...
export function registraFlush(getDataset, haLavoroNonEsportato) {
  const flush = () => {
    try {
      const d = getDataset();
      // Sincrono: l'unico che fa in tempo. Con il database aperto è la copia
      // di EMERGENZA (secondaria): se non ci sta si toglie, vedi `salvaCopia`.
      if (d) salvaCopia(d, { secondaria: modalita === 'idb' && Boolean(db) });
