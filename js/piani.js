...
/**
 * I controlli che un pezzo nuovo può PORTARSI DIETRO, cioè quelli di cui ha
 * senso chiedere «l'ultimo quando è stato fatto» leggendo la sua etichetta.
 *
 * ⛔ ROTTAMAZIONE non c'è (19/09/2026, segnalazione dell'operatore: «perché ci
 * chiede ultimo ROTTAMAZIONE?»). La fine vita non si ESEGUE, si RAGGIUNGE: è
 * una soglia contata dall'anno di costruzione, che il form chiede già. «L'ultima
 * volta che questo pezzo è stato rottamato» è una domanda senza risposta.
 *
 * E non era solo una domanda assurda: misurato su un estintore del 2023,
 * dichiarare una ROTTAMAZIONE gli faceva SPARIRE la scadenza di fine vita — da
 * 2041 a nessuna — lasciandolo «idoneo» e mai più segnalato.
 *
 * GEMELLA di `CONTROLLI_DICHIARABILI` in
 * `backend/app/services/scudo_registrazione.py`, che RIFIUTA un dichiarato non
 * ammesso. Le tiene allineate `scripts/scudo/test_dichiarabili_cross.py`.
 */
export const CONTROLLI_DICHIARABILI = ['REVISIONE', 'COLLAUDO'];
